namespace AppForJoystickCameraAndSerial.Controllers
{
    public class SerialPacketHandler
    {
        ...
        private readonly Task[] recordTasks = new Task[2];
        ...
        {
            if (Record)
                recordTasks[index] = Task.Factory.StartNew(() => StartRecord(Rx_Data, RecordDir, index));
            for (byte k = 2; k < 52; k++)
        ...
        private void StartRecord(byte[] Rx_Data, string RecordDir, int index)
        {
            ...
            for (int h = 0; h < 55; h++)
                File.AppendAllText(recordingpath, "" + Rx_Data[h] + ',');
        }
        ...
        public void SplitLookupTable(byte[] LUT, TextBox fov_TextBox, TextBox azError_TextBox, TextBox eiError_TextBox, TextBox ax_TextBox, TextBox ay_TextBox, TextBox az_TextBox)
        {
            ...
            ChangeTextBox(ax_TextBox, ((float)Data_1 / 1000).ToString());
            ChangeTextBox(ay_TextBox, ((float)Data_2 / 1000).ToString());
            ChangeTextBox(az_TextBox, ((float)Data_3 / 1000).ToString());
            ChangeTextBox(fov_TextBox, ((float)Data_4 / 1000).ToString());
            ChangeTextBox(azError_TextBox, ((float)Data_5 / 1000).ToString());
            ChangeTextBox(eiError_TextBox, ((float)Data_6 / 1000).ToString());
        }
